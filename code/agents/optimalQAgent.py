...
class OptimalQLearningAgent(Agent):
  def __init__(self, observation_space, action_space, location=(0,0), learning_rate=0.001, gamma=0.95, weight_decay=0, model_layer_size=0):
    super(OptimalQLearningAgent, self).__init__(location)
    ...
    self.q_network = QNetworkOptimalAgent(input_size, output_size, model_layer_size=model_layer_size)
  ...
  def update_q_function(self, states, actions, rewards, next_states, dones, keep_exploring=False):
  ...
  #  states = torch.tensor(states, dtype=torch.float32)  # Shape: [batch_size, state_dim]
   ...
   # next_states = torch.tensor(next_states, dtype=torch.float32)  # Shape: [batch_size, state_dim]
    ...
    if not keep_exploring:  # Decay epsilon for exploration
        self.epsilon = max(self.epsilon_min, self.epsilon * self.epsilon_decay)
    else:
       max(self.epsilon_min, self.epsilon * (self.epsilon_decay + (1-self.epsilon_decay)/2)) # decay slower!
